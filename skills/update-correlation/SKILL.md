...
Use `update-correlation` only after defining the traffic window you want to
...
`seo_describe_report` with `{"id":"update-correlation"}`, then use
`seo_run_report`, for example
`{"id":"update-correlation","params":{"site":"sc-domain:example.com","days":90,"recentDays":7,"paddingDays":2,"refresh":false}}`.
