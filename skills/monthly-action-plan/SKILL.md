...
1. Call `seo_list_reports` with `category: "workflows"` and select `monthly-action-plan`.
2. Call `seo_describe_report` with `id: "monthly-action-plan"`; confirm its current parameters.
3. Call `seo_run_report` with that id and only schema-valid parameters.

CLI parity:

```sh
seo reports describe monthly-action-plan --json
seo reports run monthly-action-plan --params '{"site":"sc-domain:example.com","month":"2026-05","limit":10,"includeBrand":false}' --json
```
