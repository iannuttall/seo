...
function rowLimit(value: number | undefined, provider: ProviderId): number {
  const limit = value ?? DEFAULT_ROW_LIMIT
  if (!Number.isSafeInteger(limit) || limit < 1 || limit > MAX_ROW_LIMIT) {
    throw new SeoError(
      'INVALID_INPUT',
      `${provider} link row limit must be between 1 and ${MAX_ROW_LIMIT}.`,
    )
  }
  return limit
}

export async function collectExternalProviderLinkEvidence(input: {
  providerId: 'ahrefs' | 'dataforseo'
  target: string
  ...
  provider: Pick<LiveLinkProvider, 'linkSummary' | 'backlinks'>
}): Promise<CollectedLinkEvidence> {
  const limit = rowLimit(input.rowLimit, input.providerId)
  const context = input.context ?? {
    reportId: 'link-evidence',
    reportRunId: randomUUID(),
  }
  const summary = await input.provider.linkSummary({
    target: input.target,
    scope: input.scope,
  ...
  const backlinks = await input.provider.backlinks({
