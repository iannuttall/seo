...
1. Call `seo_list_reports` with `category: "reporting"` and select `report-narrative`.
2. Call `seo_describe_report` with `id: "report-narrative"` before constructing parameters.
3. Call `seo_run_report` with `id: "report-narrative"` and schema-valid parameters.

CLI parity:

```sh
seo reports describe report-narrative --json
seo reports run report-narrative --params '{"site":"sc-domain:example.com","days":90,"recentDays":7,"limit":10,"includeBrand":false,"changeLimit":5}' --json
```
