...
output differs: the CLI prints `structuredContent` in JSON mode, while the
...
Search Console and Google Analytics access stays read-only. A server, container, or CI runner
