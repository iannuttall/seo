...
const expectedPages = new Map([
  ...
  ['docs/html-reports/index.html', 'https://seoskill.dev/docs/html-reports'],
  ['docs/indexnow/index.html', 'https://seoskill.dev/docs/indexnow'],
...
test('every content page has one deterministic Markdown alternative', () => {
  ...
  assert.equal(manifest.pages.length, representedHtml.length)
...
test('llms.txt is a short curated map generated from the route manifest', async () => {
  ...
  const { renderLlmsTxt } = await import('@iannuttall/seo-graph-core')
...
test('report library covers the live registry and keeps legacy routes', async () => {
  ...
  assert.match(homeHtml, new RegExp(`${liveIds.length} reports and tools`))
  assert.match(homeHtml, new RegExp(`Run ${liveIds.length} reports across`))
...
test('published report count stays tied to the implementation', async () => {
  ...
  const implementedCount = listReportDefinitions().length
  assert.equal(implementedCount, routeReportIds.length)
  assert.match(homeText, new RegExp(`${implementedCount} reports`))
})
...
test('Cloudflare limits the Worker to anonymous telemetry and static assets', () => {
  ...
  assert.equal(
    matches(headers, /X-Markdown-Tokens: \d+/g).length,
    manifest.pages.length,
  )
  for (const page of manifest.pages) {
    // Generated by @iannuttall/seo-graph-astro: the canonical Link appends
    // to the /* discovery links instead of resetting and re-adding them.
    const rule = [
      page.markdownPath,
      '  ! Vary',
...
test('telemetry prose and stats keep a stable readable first render', () => {
  ...
  // aggregate JSON instead, and every dynamic section is excluded from the
  // Markdown alternate so placeholder zeros never reach it.
  assert.match(
    stats,
  ...
  assert.doesNotMatch(statsMarkdown, /No recorded events/u)
})
