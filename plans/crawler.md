...
- [x] Implement GEO rules: no machine-readable structure, weak semantic HTML, missing author/date, not answer-ready, too thin to cite, missing `llms.txt`.
...
- [x] Keep storage shape adaptable for future hosted persistence.
...
- [x] Keep auth/data joins optional and provider-based.
...
- [x] Decide initial severity names: existing `low|medium|high` vs crawler-style `notice|warning|error`.
