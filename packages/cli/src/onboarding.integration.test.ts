...
test('Clicky detach removes one project link without needing the sitekey', async () => {
  const configDir = await mkdtemp(join(tmpdir(), 'seo-clicky-detach-config-'))
  const cacheDir = await mkdtemp(join(tmpdir(), 'seo-clicky-detach-cache-'))
  const env = { SEO_CONFIG_DIR: configDir, SEO_CACHE_DIR: cacheDir }
  ...
  try {
    ...
    const detached = JSON.parse(
      await runSeo(
        ['analytics', 'clicky', 'detach', '--project', 'example', '--json'],
        env,
      ),
    )
    ...
    const project = JSON.parse(
      await runSeo(['projects', 'show', '--id', 'example', '--json'], env),
    )
