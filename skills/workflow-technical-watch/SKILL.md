...
1. Call `seo_list_reports` with `category: "workflows"` and select `workflow-technical-watch`.
2. Call `seo_describe_report` with `id: "workflow-technical-watch"`; choose the components required.
3. Call `seo_run_report` with that id and only schema-valid parameters.

CLI parity:

```sh
seo reports describe workflow-technical-watch --json
seo reports run workflow-technical-watch --params '{"site":"sc-domain:example.com","startUrl":"https://example.com/","sitemaps":["https://example.com/sitemap.xml"],"limit":250,"dailyLimit":200,"inspectLimit":25,"recoverLinks":true}' --json
```
