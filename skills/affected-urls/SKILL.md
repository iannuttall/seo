...
Each row ties a URL to a rule, category, severity, and crawler evidence. Optional clicks, impressions, and sessions help order work, but inspect `dataSources` first: unavailable provider data must not be interpreted as a real zero. Results are sorted deterministically by severity, joined demand signals, and URL. The `selection` object reports `totalMatchedRows`, `returnedRows`, `limit`, and `truncated`; preserve it whenever the URL array is passed downstream.

Check the requested filters, selection metadata, original crawl cap, warnings, and caveats before calling the set complete. `truncated: true` means more matching issue rows exist in this report than were returned. A capped or partial crawl only supports "affected among evaluated pages." Create fixes from the observed evidence, sample several URL templates, and use `explain-crawl-issue` for rule guidance. Re-run the same bounded crawl after deployment to verify that the rule no longer triggers; disappearance from a differently scoped crawl is not verification.
