...
Run `doctor` before blaming a report for an authentication or configuration
...
`seo_describe_report` with `{"id":"doctor"}` and use the returned schema.
Finally call `seo_run_report` with `{"id":"doctor","params":{}}`. The report
accepts no parameters, so do not invent a site, project, or refresh field.

The CLI exposes the same registry and schema:

```sh
seo reports describe doctor --json
seo reports run doctor --params '{}' --json
```
...
restoring the same OAuth client used by stored tokens. Re-run `doctor`, then run
